...
/*状态机构造函数：传入参数，控制器初始化状态机中的类，姿态容器和遥控结构体初始化*/
...
void PX4CtrlFSM::process() {
  ...
  case MANUAL_CTRL: //默认值
  {
    ...
    /*第一次一定会进入到上面的case，而如果无法进入if语句则state不会切换则不会起飞，进入if里面一两种可能悬停和自动起飞，他们的实际生效在下面的case体现
    特殊的自动起飞会检查是否上锁*/
  ...
  case AUTO_HOVER: {
    ...
    /*如果数据异常重新获取，如果不在悬停姿态，回去检测是否收到进入悬停*/
    // cmd触发接收轨迹
