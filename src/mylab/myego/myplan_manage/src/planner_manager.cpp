...
bool EGOPlannerManager::reboundReplan(Eigen::Vector3d start_pt,
  ...
  double ts = (start_pt - local_target_pt).norm() > 0.1
                  ...
                  : pp_.ctrl_pt_dist / pp_.max_vel_ *
                        5; // pp_.ctrl_pt_dist / pp_.max_vel_ is too tense, and
    ...
    } else // Initial path generated from previous trajectory.
    {
      ...
      for (t = t_cur; t < local_data_.duration_ + 1e-3; t += ts) {
        ...
        if (t > t_cur) {
          pseudo_arc_length.push_back(
              (segment_point.back() - segment_point[segment_point.size() - 2])
                  .norm() +
      ...
      if (poly_time > ts) {
        PolynomialTraj gl_traj = PolynomialTraj::one_segment_traj_gen(
            ...
            local_data_.acceleration_traj_.evaluateDeBoorT(t), local_target_pt,
            local_target_vel, Eigen::Vector3d::Zero(), poly_time);
      ...
      double cps_dist =
          pp_.ctrl_pt_dist * 1.5; // cps_dist will be divided by 1.5 in the next
      size_t id = 0;
      do {
        cps_dist /= 1.5;
        point_set.clear();
      ...
      if (point_set.size() > pp_.planning_horizen_ / pp_.ctrl_pt_dist *
                                 3) // The initial path is unnormally too long!
      {
  ...
  segments =
      bspline_optimizer_->initControlPoints(ctrl_pts, true); // 通过A star 搜索
  ...
  // Note: Only adjust time in single drone mode. But we still allow drone_0 to
...
bool EGOPlannerManager::refineTrajAlgo(
    UniformBspline &traj, vector<Eigen::Vector3d> &start_end_derivative,
  ...
  bool success = bspline_optimizer_->BsplineOptimizeTrajRefine(
      ctrl_pts, ts, optimal_control_points);

  return success;
}

void EGOPlannerManager::reparamBspline(
    UniformBspline &bspline, vector<Eigen::Vector3d> &start_end_derivative,
  ...
  UniformBspline::parameterizeToBspline(dt, point_set, start_end_derivative,
                                        ctrl_pts);
}
...
void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh,
  ...
  // obj_predictor_->init();
  // obj_pub_ = nh.advertise<visualization_msgs::Marker>("/dynamic/obj_prdi",
  // 10); // zx-todo
  ...
  bspline_optimizer_->a_star_->initGridMap(grid_map_,
...
bool EGOPlannerManager::planGlobalTraj(const Eigen::Vector3d &start_pos,
  ...
  PolynomialTraj gl_traj;
  if (pos.cols() >= 3)
    gl_traj = PolynomialTraj::minSnapTraj(pos, start_vel, end_vel, start_acc,
                                          end_acc, time);
  else if (pos.cols() == 2)
    gl_traj = PolynomialTraj::one_segment_traj_gen(
...
bool EGOPlannerManager::planGlobalTrajWaypoints(
  ...
  if (pos.cols() >= 3)
    ...
    gl_traj = PolynomialTraj::one_segment_traj_gen(
        start_pos, start_vel, start_acc, pos.col(1), end_vel, end_acc, time(0));
  else
    return false;
...
bool EGOPlannerManager::checkCollision(int drone_id) {
  if (local_data_.start_time_.toSec() <
  ...
  double t_end =
      min(my_traj_start_time + local_data_.duration_ * 2 / 3,
  ...
  for (double t = t_start; t < t_end; t += 0.03) {
    if ((local_data_.position_traj_.evaluateDeBoorT(t - my_traj_start_time) -
         swarm_trajs_buf_[drone_id].position_traj_.evaluateDeBoorT(
             t - other_traj_start_time))
