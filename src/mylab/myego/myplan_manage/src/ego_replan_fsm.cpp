...
void EGOReplanFSM::init(ros::NodeHandle &nh) {
  ...
  if (planner_manager_->pp_.drone_id >= 1) {
    string sub_topic_name = string("/drone_") +
                            std::to_string(planner_manager_->pp_.drone_id - 1) +
  ...
  string pub_topic_name = string("/drone_") +
                          std::to_string(planner_manager_->pp_.drone_id) +
                          string("_planning/swarm_trajs");
  swarm_trajs_pub_ =
      nh.advertise<mytraj_utils::MultiBsplines>(pub_topic_name.c_str(), 10);

  broadcast_bspline_pub_ = nh.advertise<mytraj_utils::Bspline>(
      "planning/broadcast_bspline_from_planner", 10);
  broadcast_bspline_sub_ =
      nh.subscribe("planning/broadcast_bspline_to_planner", 100,
                   &EGOReplanFSM::BroadcastBsplineCallback, this,
                   ros::TransportHints().tcpNoDelay());

  bspline_pub_ = nh.advertise<mytraj_utils::Bspline>("planning/bspline", 10);
  data_disp_pub_ =
      nh.advertise<mytraj_utils::DataDisp>("planning/data_display", 100);
...
void EGOReplanFSM::readGivenWps() {
  ...
  for (size_t i = 0; i < (size_t)waypoint_num_; i++) {
    visualization_->displayGoalPoint(wps_[i], Eigen::Vector4d(0, 0.5, 0.5, 1),
                                     0.3, i);
    ros::Duration(0.001).sleep();
  }
  ...
  //   int i_end = floor(planner_manager_->global_data_.global_duration_ /
  //   step_size_t); std::vector<Eigen::Vector3d> gloabl_traj(i_end); for (int i
  //   = 0; i < i_end; i++)
  //   {
  //     gloabl_traj[i] = planner_manager_->global_data_.global_traj_.evaluate(i
  //     * step_size_t);
  //   }
  ...
  //   0.3, 0); ros::Duration(0.001).sleep();
...
void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp) {
  ...
  // visualization_->displayGoalPoint(next_wp, Eigen::Vector4d(0, 0.5, 0.5, 1),
  // 0.3, 0);
...
void EGOReplanFSM::BroadcastBsplineCallback(
    const mytraj_utils::BsplinePtr &msg) {
  ...
  // planner_manager_->swarm_trajs_buf_[id].start_time_ = ros::Time::now(); //
  // Un-reliable time sync
...
void EGOReplanFSM::swarmTrajsCallback(
    const mytraj_utils::MultiBsplinesPtr &msg) {

  multi_bspline_msgs_buf_.traj.clear();
  multi_bspline_msgs_buf_ = *msg;

  // cout << "\033[45;33mmulti_bspline_msgs_buf.drone_id_from=" <<
  // multi_bspline_msgs_buf_.drone_id_from << "
  // multi_bspline_msgs_buf_.traj.size()=" <<
  // multi_bspline_msgs_buf_.traj.size() << "\033[0m" << endl;
  ...
  if ((int)msg->traj.size() !=
      msg->drone_id_from + 1) // drone_id must start from 0
  {
    ROS_ERROR(
        "Wrong trajectory size! msg->traj.size()=%d, msg->drone_id_from+1=%d",
...
void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e) {
  ...
  {
    // cout << "id=" << planner_manager_->pp_.drone_id << "
    // have_recv_pre_agent_=" << have_recv_pre_agent_ << endl;
    if (planner_manager_->pp_.drone_id <= 0 ||
  ...
  case EXEC_TRAJ: {
    ...
    if ((target_type_ == TARGET_TYPE::PRESET_TARGET) &&
      ...
      wp_id_++;
      planNextWaypoint(wps_[wp_id_]);
    } else if ((local_target_pt_ - end_pt_).norm() <
               1e-3) // close to the global target
    {
  ...
  for (double t = t_cur; t < info->duration_; t += time_step) {
    ...
    for (size_t id = 0; id < planner_manager_->swarm_trajs_buf_.size(); id++) {
      if ((planner_manager_->swarm_trajs_buf_.at(id).drone_id != (int)id) ||
          (planner_manager_->swarm_trajs_buf_.at(id).drone_id ==
      ...
      Eigen::Vector3d swarm_pridicted =
          planner_manager_->swarm_trajs_buf_.at(id)
              .position_traj_.evaluateDeBoorT(t_X);
...
bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init,
  ...
  bool plan_and_refine_success = planner_manager_->reboundReplan(
...
void EGOReplanFSM::publishSwarmTrajs(bool startup_pub) {
  ...
  if (startup_pub) {
    multi_bspline_msgs_buf_.drone_id_from =
        planner_manager_->pp_.drone_id; // zx-todo
    if ((int)multi_bspline_msgs_buf_.traj.size() ==
        planner_manager_->pp_.drone_id + 1) {
      multi_bspline_msgs_buf_.traj.back() = bspline;
    } else if ((int)multi_bspline_msgs_buf_.traj.size() ==
               planner_manager_->pp_.drone_id) {
      multi_bspline_msgs_buf_.traj.push_back(bspline);
    } else {
      ROS_ERROR(
          "Wrong traj nums and drone_id pair!!! traj.size()=%d, drone_id=%d",
          (int)multi_bspline_msgs_buf_.traj.size(),
          planner_manager_->pp_.drone_id);
      // return plan_and_refine_success;
    }
