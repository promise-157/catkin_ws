...
Eigen::MatrixXd UniformBspline::getDerivativeControlPoints() {
  ...
  for (int i = 0; i < ctp.cols(); ++i) {
    ctp.col(i) = p_ * (control_points_.col(i + 1) - control_points_.col(i)) /
                 (u_(i + p_ + 1) - u_(i + 1));
  }
  return ctp;
}
...
void UniformBspline::setPhysicalLimits(const double &vel, const double &acc,
...
bool UniformBspline::checkFeasibility(double &ratio, bool show) {
  ...
  for (int i = 0; i < P.cols() - 1; ++i) {
    Eigen::VectorXd vel =
        p_ * (P.col(i + 1) - P.col(i)) / (u_(i + p_ + 1) - u_(i + 1));
