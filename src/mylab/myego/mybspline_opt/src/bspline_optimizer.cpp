...
#include <mybspline_opt/lbfgs.hpp>
namespace myego {
...
void BsplineOptimizer::setEnvironment(const GridMap::Ptr &map,
                                      const myego::ObjPredictor::Ptr mov_obj) {
  this->grid_map_ = map;
...
BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments) {
  ...
  const double CTRL_PT_DIST =
      (cps_.points.col(0) - cps_.points.col(cps_.size - 1)).norm() /
      (cps_.size - 1);
  ...
  for (int i = 0; i < seg_upbound; i++) {
    ...
    //   {
    //     cout << "###" << RichInfoOneSeg_temp.points.col(k).transpose() <<
    //     endl; for (int k2 = 0; k2 < RichInfoOneSeg_temp.base_point[k].size();
    //     k2++)
    //     {
    //       cout << "      " <<
    //       RichInfoOneSeg_temp.base_point[k][k2].transpose() << " @ " <<
  ...
  for (int i = 0; i < seg_upbound; i++) {
    ...
    if (RichInfoSegs[i].first.size > 1) {
      ...
      for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++) {
        ...
        for (double a = 1; a > 0; a -= step_size) {
          ...
          // cout << " " << grid_map_->getInflateOccupancy(pt) << " pt=" <<
          // pt.transpose() << endl;
    ...
    exit_multi_loop2:;
      ...
      if (occ_start_id == -1 || occ_end_id == -1) {
        ...
        // in obstacles, which is not allowed. ROS_WARN("What? occ_start_id=%d,
        // occ_end_id=%d", occ_start_id, occ_end_id);
        ...
        //   {
        //     cout << "###" << RichInfoSegs[i].first.points.col(k).transpose()
        //     << endl; for (int k2 = 0; k2 <
        //     RichInfoSegs[i].first.base_point[k].size(); k2++)
        //     {
        //       cout << "      " <<
        //       RichInfoSegs[i].first.base_point[k][k2].transpose() << " @ " <<
      ...
      for (int j = occ_start_id; j <= occ_end_id; j++) {
        ...
        if (RichInfoSegs[i].first.base_point[j].size() != 1) {
          ...
          cout << setprecision(5);
          cout << "cps_" << endl;
          cout << " clearance=" << cps_.clearance << " cps.size=" << cps_.size
               << endl;
        ...
        if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
        {
          ...
          if (l > l_upbound) {
            ROS_WARN("Can't find the new base points at the opposite within "
                     "the threshold. i=%d, j=%d",
                     i, j);
    ...
    exit_multi_loop3:;
      ...
      cout << "i=" << i << " RichInfoSegs.size()=" << RichInfoSegs.size()
           << endl;
      cout << "RichInfoSegs[i].first.size=" << RichInfoSegs[i].first.size
           << endl;
      cout << "RichInfoSegs[i].first.direction.size()="
           << RichInfoSegs[i].first.direction.size() << endl;
      cout << "RichInfoSegs[i].first.direction[0].size()="
           << RichInfoSegs[i].first.direction[0].size() << endl;
      cout << "RichInfoSegs[i].first.points.cols()="
           << RichInfoSegs[i].first.points.cols() << endl;
      cout << "RichInfoSegs[i].first.base_point.size()="
           << RichInfoSegs[i].first.base_point.size() << endl;
      cout << "RichInfoSegs[i].first.base_point[0].size()="
           << RichInfoSegs[i].first.base_point[0].size() << endl;
      ...
      if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
      {
        ...
        if (l > l_upbound) {
          ROS_WARN("Can't find the new base points at the opposite within the "
                   "threshold, 2. i=%d",
                   i);
  ...
  for (int i = 0; i < max_traj_nums; i++) {
    ...
    while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.size) {
      ...
      // {
      //   cout << "seg_id=" << seg_id << " RichInfoSegs.size()=" <<
      //   RichInfoSegs.size() << endl;
      // }
      // if ( cp_id >= cps_.base_point.size() )
      // {
      //   cout << "cp_id=" << cp_id << " cps_.base_point.size()=" <<
      //   cps_.base_point.size() << endl;
      // }
      // if ( cp_of_seg_id >= RichInfoSegs[seg_id].first.base_point.size() )
      // {
      //   cout << "cp_of_seg_id=" << cp_of_seg_id << "
      //   RichInfoSegs[seg_id].first.base_point.size()=" <<
      ...
      if (seg_id >= seg_upbound || cp_id < segments[seg_id].first ||
        ...
        ROS_ERROR("Shold not happen!!!!, cp_id=%d, seg_id=%d, "
                  "segments.front().first=%d, segments.back().second=%d, "
 ...
 * It was written separately, just because I did it once and it has been running
 * stably since March 2020. But I will merge then someday.*/
std::vector<std::pair<int, int>>
BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points,
  ...
  double step_size = grid_map_->getResolution() /
                     ((init_points.col(0) - init_points.rightCols(1)).norm() /
                      (init_points.cols() - 1)) /
                     1.5;
  ...
  for (int i = order_; i <= i_end; ++i) {
    ...
    for (double a = 1.0; a > 0.0; a -= step_size) {
      ...
      // cout << " " << occ;
      // cout << setprecision(5);
      // cout << (a * init_points.col(i-1) + (1-a) *
      // init_points.col(i)).transpose() << " occ1=" << occ << endl;
      ...
      if (flag_got_end_maybe && (same_occ_state_times > ENOUGH_INTERVAL ||
                                 (i == (int)init_points.cols() - order_))) {
  ...
  // {
  //   cout << "segment_ids=" << segment_ids[i].first << " ~ " <<
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    // cout << "final:" << "i = " << i << " first = " <<
    // adjusted_segment_ids[i].first << " second = " <<
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
      ...
      while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size()) {
        ...
        if (val * last_val <= 0 &&
            (abs(val) > 0 ||
             abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
        {
          intersection_point =
          ...
          // cout << "i=" << i << " j=" << j << " Astar_id=" << Astar_id << "
          // last_Astar_id=" << last_Astar_id << " intersection_point = " <<
          // intersection_point.transpose() << endl;
...
int BsplineOptimizer::earlyExit(void *func_data, const double *x,
                                const double *g, const double fx,
                                const double xnorm, const double gnorm,
                                const double step, int n, int k, int ls) {
  BsplineOptimizer *opt = reinterpret_cast<BsplineOptimizer *>(func_data);
  // cout << "k=" << k << endl;
  // cout << "opt->flag_continue_to_optimize_=" <<
  // opt->flag_continue_to_optimize_ << endl;
  return (opt->force_stop_type_ == STOP_FOR_ERROR ||
          opt->force_stop_type_ == STOP_FOR_REBOUND);
}

double BsplineOptimizer::costFunctionRebound(void *func_data, const double *x,
...
double BsplineOptimizer::costFunctionRefine(void *func_data, const double *x,
                                            double *grad, const int n) {
  BsplineOptimizer *opt = reinterpret_cast<BsplineOptimizer *>(func_data);
...
void BsplineOptimizer::calcSwarmCost(const Eigen::MatrixXd &q, double &cost,
                                     Eigen::MatrixXd &gradient) {
  cost = 0.0;
  int end_idx = q.cols() - order_ -
                (double)(q.cols() - 2 * order_) * 1.0 /
                    3.0; // Only check the first 2/3 points
  const double CLEARANCE = swarm_clearance_ * 2;
  double t_now = ros::Time::now().toSec();
  constexpr double a = 2.0, b = 1.0, inv_a2 = 1 / a / a, inv_b2 = 1 / b / b;

  for (int i = order_; i < end_idx; i++) {
    double glb_time = t_now + ((double)(order_ - 1) / 2 + (i - order_ + 1)) *
                                  bspline_interval_;
    ...
    for (size_t id = 0; id < swarm_trajs_->size(); id++) {
      ...
      if (glb_time < traj_i_satrt_time + swarm_trajs_->at(id).duration_ - 0.1) {
         ...
         */
        Eigen::Vector3d swarm_prid =
            swarm_trajs_->at(id).position_traj_.evaluateDeBoorT(
...
void BsplineOptimizer::calcMovingObjCost(const Eigen::MatrixXd &q, double &cost,
                                         Eigen::MatrixXd &gradient) {
  cost = 0.0;
  int end_idx = q.cols() - order_;
  ...
  for (int i = order_; i < end_idx; i++) {
    double time =
        ((double)(order_ - 1) / 2 + (i - order_ + 1)) * bspline_interval_;

    for (int id = 0; id < moving_objs_->getObjNums(); id++) {
      Eigen::Vector3d obj_prid =
          moving_objs_->evaluateConstVel(id, t_now + time);
      double dist = (cps_.points.col(i) - obj_prid).norm();
      // cout /*<< "cps_.points.col(i)=" << cps_.points.col(i).transpose()*/ <<
      // " moving_objs_=" << obj_prid.transpose() << " dist=" << dist << endl;
    ...
    // cout << "time=" << time << " i=" << i << " order_=" << order_ << "
    // end_idx=" << end_idx << endl; cout << "--" << endl;
  }
  // cout << "---------------" << endl;
}
...
void BsplineOptimizer::calcDistanceCostRebound(const Eigen::MatrixXd &q,
  ...
  double a = 3 * demarcation, b = -3 * pow(demarcation, 2),
         c = pow(demarcation, 3);

  force_stop_type_ = DONT_STOP;
  if (iter_num > 3 && smoothness_cost / (cps_.size - 2 * order_) <
                          0.1) // 0.1 is an experimental value that indicates
  ...
  for (auto i = order_; i < end_idx; ++i) {
    for (size_t j = 0; j < cps_.direction[i].size(); ++j) {
      double dist = (cps_.points.col(i) - cps_.base_point[i][j])
...
void BsplineOptimizer::calcTerminalCost(const Eigen::MatrixXd &q, double &cost,
                                        Eigen::MatrixXd &gradient) {
  cost = 0.0;
...
void BsplineOptimizer::calcFeasibilityCost(const Eigen::MatrixXd &q,
  ...
  double ar = 3 * demarcation, br = -3 * pow(demarcation, 2),
         cr = pow(demarcation, 3);
    ...
    for (int j = 0; j < 3; j++) {
      if (vi(j) > max_vel_ + demarcation) {
        ...
        cost += (ar * diff * diff + br * diff + cr) *
                ts_inv3; // multiply ts_inv3 to make vel and acc has similar
                         // magnitude
  ...
  for (int i = 0; i < q.cols() - 1; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      if (vi(j) > max_vel_) {
        ...
        // cout << vi(j) << endl;
        cost +=
            pow(vi(j) - max_vel_, 2) * ts_inv2; // multiply ts_inv3 to make vel
                                                // and acc has similar magnitude
...
bool BsplineOptimizer::check_collision_and_rebound(void) {
  ...
  for (int i = order_ - 1; i <= i_end; ++i) {
      ...
      for (size_t k = 0; k < cps_.direction[i].size(); ++k) {
        ...
        if ((cps_.points.col(i) - cps_.base_point[i][k])
            ...
            1 * grid_map_->getResolution()) // current point is outside all the
                                            // collision_points.
        {
          occ = false; // Not really takes effect, just for better hunman
                       // understanding.
          break;
        }
      }
    ...
    if (occ) {
      ...
      {
        ROS_WARN("WARN! terminal point of the current trajectory is in "
                 "obstacle, skip this planning.");
...
bool BsplineOptimizer::BsplineOptimizeTrajRefine(
    const Eigen::MatrixXd &init_points, const double ts,
...
bool BsplineOptimizer::rebound_optimize(double &final_cost) {
  ...
  do {
    ...
    mylbfgs::lbfgs_parameter_t lbfgs_params;
    mylbfgs::lbfgs_load_default_parameters(&lbfgs_params);
    ...
    /* ---------- optimize ---------- */
    t1 = ros::Time::now();
    int result = mylbfgs::lbfgs_optimize(
        variable_num_, q, &final_cost, BsplineOptimizer::costFunctionRebound,
    ...
    if (result == mylbfgs::LBFGS_CONVERGENCE ||
        result == mylbfgs::LBFGSERR_MAXIMUMITERATION ||
        result == mylbfgs::LBFGS_ALREADY_MINIMIZED ||
        result == mylbfgs::LBFGS_STOP) {
      // ROS_WARN("Solver error in planning!, return = %s",
      // lbfgs::lbfgs_strerror(result));
      flag_force_return = false;

      /*** collision check, phase 1 ***/
      if ((min_ellip_dist_ != INIT_min_ellip_dist_) &&
        ...
        printf("\033[32miter(+1)=%d,time(ms)=%5.3f, swarm too close, keep "
               "optimizing\n\033[0m",
               iter_num_, time_ms);
      ...
      for (double t = tm; t < tmp * 2 / 3;
           t += t_step) // Only check the closest 2/3 partition of the whole
                        // trajectory.
      {
        flag_occ = grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t));
        if (flag_occ) {
          // cout << "hit_obs, t=" << t << " P=" <<
          // traj.evaluateDeBoorT(t).transpose() << endl;
          ...
          if (t <= bspline_interval_) // First 3 control points in obstacles!
          {
            ...
            ROS_WARN("First 3 control points in obstacles! return false, t=%f",
                     t);
            return false;
          }
      ...
      // cout << "XXXXXX" << ((cps_.points.col(cps_.points.cols()-1) +
      // 4*cps_.points.col(cps_.points.cols()-2) +
      // cps_.points.col(cps_.points.cols()-3))/6 - local_target_pt_).norm() <<
      // endl;
...
//#define USE_SECOND_CLEARENCE_CHECK
      ...
      Eigen::Vector3d start_end_vec =
          traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm);
      ...
      if (!flag_occ) {
        printf("\033[32miter(+1)=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5."
               "3f\n\033[0m",
      ...
      } else // restart
      {
        ...
        printf("\033[32miter(+1)=%d,time(ms)=%5.3f, collided, keep "
               "optimizing\n\033[0m",
               iter_num_, time_ms);
      }
    } else if (result == mylbfgs::LBFGSERR_CANCELED) {
      flag_force_return = true;
      rebound_times++;
      cout << "iter=" << iter_num_ << ",time(ms)=" << time_ms << ",rebound."
           << endl;
    } else {
      ROS_WARN("Solver error. Return = %d, %s. Skip this planning.", result,
               mylbfgs::lbfgs_strerror(result));
      // while (ros::ok());
    }

  } while (((flag_occ || ((min_ellip_dist_ != INIT_min_ellip_dist_) &&
                          (min_ellip_dist_ > swarm_clearance_))) &&
...
bool BsplineOptimizer::refine_optimize() {
  ...
  do {
    ...
    int result = mylbfgs::lbfgs_optimize(variable_num_, q, &final_cost,
    ...
    if (result == mylbfgs::LBFGS_CONVERGENCE ||
      ...
      ROS_ERROR("Solver error in refining!, return = %d, %s", result,
                mylbfgs::lbfgs_strerror(result));
    }
    ...
    double t_step =
        ...
        ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() /
         grid_map_
             ->getResolution()); // Step size is defined as the maximum size
                                 // that can passes throgth every gird.
    for (double t = tm; t < tmp * 2 / 3; t += t_step) {
      if (grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t))) {
        // cout << "Refined traj hit_obs, t=" << t << " P=" <<
        // traj.evaluateDeBoorT(t).transpose() << endl;
...
void BsplineOptimizer::combineCostRebound(const double *x, double *grad,
  ...
  calcDistanceCostRebound(cps_.points, f_distance, g_distance, iter_num_,
                          f_smoothness);
  ...
  f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance +
              ...
              lambda2_ * f_terminal;
  // f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_
  // * f_feasibility + new_lambda2_ * f_mov_objs; printf("origin %f %f %f %f\n",
  // f_smoothness, f_distance, f_feasibility, f_combine);

  Eigen::MatrixXd grad_3D =
      lambda1_ * g_smoothness + new_lambda2_ * g_distance +
      lambda3_ * g_feasibility + new_lambda2_ * g_swarm + lambda2_ * g_terminal;
  // Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ *
  // g_distance + lambda3_ * g_feasibility + new_lambda2_ * g_mov_objs;
  memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
}

void BsplineOptimizer::combineCostRefine(const double *x, double *grad,
                                         double &f_combine, const int n) {
  ...
  f_combine =
      lambda1_ * f_smoothness + lambda4_ * f_fitness + lambda3_ * f_feasibility;
  // printf("origin %f %f %f %f\n", f_smoothness, f_fitness, f_feasibility,
  // f_combine);
