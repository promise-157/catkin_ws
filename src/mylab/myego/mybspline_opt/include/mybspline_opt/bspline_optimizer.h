#ifndef MYBSPLINE_OPTIMIZER_H_
#define MYBSPLINE_OPTIMIZER_H_
#include <Eigen/Dense>
#include <mypath_searching/dyn_a_star.h>
#include <myplan_env/grid_map.h>
#include <myplan_env/obj_predictor.h>
...
public:
  ...
  void segment(ControlPoints &buf, const int start, const int end) {
    ...
    for (int i = start; i <= end; i++) {
      ...
      // {
      //   ROS_ERROR("buf.base_point[i - start].size()=%d,
      //   base_point[i].size()=%d", buf.base_point[i - start].size(),
      //   base_point[i].size());
      // }
    }
...
public:
  ...
  initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init = true);
...
private:
  ...
  myego::ObjPredictor::Ptr moving_objs_;
...
#define INIT_min_ellip_dist_ 123456789.0123456789
  ...
  void calcTerminalCost(const Eigen::MatrixXd &q, double &cost,
                        Eigen::MatrixXd &gradient);
  ...
  void calcMovingObjCost(const Eigen::MatrixXd &q, double &cost,
                         Eigen::MatrixXd &gradient);
  void calcSwarmCost(const Eigen::MatrixXd &q, double &cost,
                     Eigen::MatrixXd &gradient);
  void calcFitnessCost(const Eigen::MatrixXd &q, double &cost,
                       Eigen::MatrixXd &gradient);
  bool check_collision_and_rebound(void);

  static int earlyExit(void *func_data, const double *x, const double *g,
                       const double fx, const double xnorm, const double gnorm,
                       const double step, int n, int k, int ls);
  static double costFunctionRebound(void *func_data, const double *x,
                                    double *grad, const int n);
  static double costFunctionRefine(void *func_data, const double *x,
                                   double *grad, const int n);

  bool rebound_optimize(double &final_cost);
  bool refine_optimize();
  void combineCostRebound(const double *x, double *grad, double &f_combine,
                          const int n);
  void combineCostRefine(const double *x, double *grad, double &f_combine,
                         const int n);
