 ...
 * efficient linesearch. If it is not used, just set it NULL or nullptr.
 *
 *  @param  instance    The user data sent for lbfgs_optimize() function by the
 * client.
...
inline int line_search_morethuente(int n, double *x, double *f, double *g,
                                   ...
                                   const double *gp, const double *stpmin,
                                   const double *stpmax, callback_data_t *cd,
  ...
  */
  ...
  for (;;) {
    ...
    if (cd->proc_progress) {
      ...
      vec2norm(&xnorm, x, n);
      vec2norm(&gnorm, g, n);
      if (cd->proc_progress(cd->instance, x, g, fx, xnorm, gnorm, *stp, cd->n,
                            0, 0)) {
    ...
    *f = cd->proc_evaluate(cd->instance, x, g, cd->n);
    ...
    if (*f <= ftest1 && fabs(dg) <= param->s_curv_coeff * (-dginit)) {
      /* The sufficient decrease condition and the strong curvature condition
       * hold. */
      return count;
    }
  ...
  if (gnorm / xnorm <= param.g_epsilon) {
    ...
    */
    ...
    while (loop == 1) {
      ...
      ls = line_search_morethuente(n, x, &fx, g, d, &step, xp, gp, &step_min,
                                   &step_max, &cd, &param);
      ...
      // if (cd.proc_progress)
      // {
      //     if ((ret = cd.proc_progress(cd.instance, x, g, fx, xnorm, gnorm,
      //     step, cd.n, k, ls)))
      //     {
      //         loop = 0;
      //         continue;
