#ifndef MYTEST__ROSCALLBACK_H
#define MYTEST__ROSCALLBACK_H
#include "mytest/fake_rcConfig.h"
#include <Eigen/Dense>
...
public:
  ...
  RC_Data_t();
  void check_validity();
  bool check_centered(); //貌似没用到
  void feed(mavros_msgs::RCInConstPtr pMsg);
  bool is_received(const ros::Time &now_time); //貌似没用到
};
...
public:
  ...
  mytest::fake_rcConfig msg;

  Dynamic_Data_t();
  void feed(mytest::fake_rcConfig &config);
};
