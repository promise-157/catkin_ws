...
#include "mypose_utils/pose_utils.h"
...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg) {
  ...
  if (tf45) {
    ...
    string base_s = _drone_id == -1
                        ? string("base")
                        : string("base") + std::to_string(_drone_id);
    string laser_s = _drone_id == -1
                         ? string("laser")
                         : string("laser") + std::to_string(_drone_id);
    string vision_s = _drone_id == -1
                          ? string("vision")
                          : string("vision") + std::to_string(_drone_id);
    string height_s = _drone_id == -1
                          ? string("height")
                          : string("height") + std::to_string(_drone_id);

    broadcaster->sendTransform(tf::StampedTransform(
        transform, msg->header.stamp, string("world"), base_s));
...
int main(int argc, char **argv) {
  ...
  n.param(
      ...
      std::string("package://myodom_visualization/meshes/hummingbird.mesh"));
